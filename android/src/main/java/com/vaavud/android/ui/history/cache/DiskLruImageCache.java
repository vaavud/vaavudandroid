...
public class DiskLruImageCache implements ImageCache {
	    ...
	    public void putBitmap( String key, Bitmap data ) {
	        ...
	        try {
	            ...
	            if ( editor == null ) {
//	            	Log.d("DISK_LRU_IMAGE", "Editor Null. Bitmap key: "+ key);
	                return;
	            }

	            if( writeBitmapToFile( data, editor ) ) {
//	            	Log.d("DISK_LRU_IMAGE", "Write to bitmap key: "+ key);
	                mDiskCache.flush();
	                editor.commit();
	                
	            } else {
//	            	Log.d("DISK_LRU_IMAGE", "Abort Editor. Bitmap key: "+ key);
	                editor.abort();
	            }
	        } catch (IOException e) {
//	        	Log.d( "DISK_LRU_IMAGE", "ERROR on: image put on disk cache " + key );
	            try {
	                if ( editor != null ) {
	                    editor.abort();
	                }
	    ...
	    public Bitmap getBitmap( String key ) {
	        ...
	        try {
	        	key=Integer.toString(key.hashCode());
//	        	Log.d("DISK_LRU_IMAGE", "Image Loader cache key: "+ key);
	        	
	            snapshot = mDiskCache.get( key );
	            
	            if ( snapshot == null ) {
//	            	Log.d("DISK_LRU_IMAGE", "SNAPSHOT IS NULL "+ key);
	                return null;
	            }
	            
//	            Log.d("DISK_LRU_IMAGE","SNAPSHOT IS NOT NULL "+ key);
