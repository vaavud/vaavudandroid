...
//			Log.d("MainActivity","No Compass");
		...
		if (((VaavudApplication)getApplication()).isUserLogged()){
			userStatus[0] = false;userStatus[1] = false;userStatus[2] = true;
...
//		        	Log.d(TAG,"After Register Device");
					userStatus[0] = false;userStatus[1] = false;userStatus[2] = false;
		...
		if (lastRegisterDevice == null || (System.currentTimeMillis() - lastRegisterDevice.getTime()) > GRACE_TIME_BETWEEN_REGISTER_DEVICE_MS) {
			...
			if (Device.getInstance(this).isMixpanelEnabled()){
				...
				try {
					...
					if (user.getFacebookAccessToken()!=null && user.getFacebookAccessToken().length()>0) props.put("Facebook", true);
...
//			Log.d(TAG,"On Menu Option Selected: SignUp");
				userStatus[0] = true;userStatus[1] = false;userStatus[2] = false;
				actionBarPosition=HISTORY_TAB;
				break;
			case 1:
				//LogIn
//			Log.d(TAG,"On Menu Option Selected: LogIn");
				userStatus[0] = false;userStatus[1] = true;userStatus[2] = false;
				actionBarPosition=HISTORY_TAB;
				break;
			case 2:
				//History
		...
		public Fragment getItem(int item) {
			switch (item) {
				...
				case HISTORY_TAB:
					 ...
					 * userStatus = {false,true,false} -> User choose log in -> LogIn Fragment
					 */
					if (userStatus[2]) return Fragment.instantiate(MainActivity.this, HistoryFragment.class.getName());
					else if(!userStatus[1] && userStatus[0]) return Fragment.instantiate(MainActivity.this, SignUpFragment.class.getName());
					else if(userStatus[1] && !userStatus[0]) return Fragment.instantiate(MainActivity.this, LoginFragment.class.getName());
					else return Fragment.instantiate(MainActivity.this, SelectorFragment.class.getName());
				default:
