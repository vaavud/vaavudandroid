 ...
 */
public abstract class S3TransferModel {
    ...
    private static LinkedHashMap<Integer, S3TransferModel> sModels =
            new LinkedHashMap<Integer, S3TransferModel>();
    ...
    public static S3TransferModel getTransferModel(int id) {
        return sModels.get(id);
    }

    public static S3TransferModel[] getAllTransfers() {
    	S3TransferModel[] models = new S3TransferModel[sModels.size()];
        return sModels.values().toArray(models);
    }
    
    public static void resetTransfers(){
    	sModels = new LinkedHashMap<Integer, S3TransferModel>(); 
    }

    public S3TransferModel(Context context, Uri uri, TransferManager manager) {
        mContext = context;
        mUri = uri;
        mManager = manager;
        String uriString = uri.toString();
        mFileName = S3Util.getFileName(uriString);
        mId = sNextId++;
