...
//		Log.d("Device","Creation Time: "+creationTime);
				...
				sleipnirVolume = db.getPropertyAsFloat(KEY_PLAYER_VOLUME);

				sleipnirEncoderCoefficients = db.getPropertyAsFloatObjectArray(KEY_CALIBRATION_COEFFICENTS);
...
//		Log.d(TAG,"Renew UUID: "+uuid);
		...
		public void setPlayerVolume(Context context, float v){
				sleipnirVolume = v;
				VaavudDatabase.getInstance(context).setPropertyAsFloat(KEY_PLAYER_VOLUME, sleipnirVolume);
		}
		...
		public void setCalibrationCoefficients(Context context,Float[] calibrationCoefficients) {
				this.sleipnirEncoderCoefficients = calibrationCoefficients;
				VaavudDatabase.getInstance(context).setPropertyAsFloatObjectArray(KEY_CALIBRATION_COEFFICENTS, calibrationCoefficients);
		}
}
