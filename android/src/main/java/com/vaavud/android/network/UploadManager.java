...
//									Log.e("UploadManager", "Got error from server uploading: " + error.getMessage());
		...
		private void uploadMagneticData() {
				if (device.getUploadMagneticData()) {
												...
												if (historyMeasurementsResponseListener != null) {
														...
														if (histJson != null && histJson.size() > 0) {
																...
																for (int i = 0; i < histJson.size(); i++) {
																		...
																		if (!(obj.get("windSpeedAvg") instanceof JsonNull))
																				tmp.setWindSpeedAvg(obj.get("windSpeedAvg").getAsFloat());
																		if (!(obj.get("windSpeedMax") instanceof JsonNull))
																				tmp.setWindSpeedMax(obj.get("windSpeedMax").getAsFloat());
																		if (!(obj.get("windDirection") instanceof JsonNull))
																				tmp.setWindDirection(obj.get("windDirection").getAsFloat());
